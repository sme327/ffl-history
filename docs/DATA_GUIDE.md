...
| Gap | Detail |
|---|---|
| ~~Consolation matchup rows~~ | Outdated — verified 2026-09-09 that `weekly_matchups.csv` DOES include consolation-bracket games in playoff weeks (e.g. 2001 wk16's consolation semis), flagged `is_playoff: true` like bracket games. That's why a naive playoff-week win count exceeds `playoff_games.csv`'s championship-bracket record — and why the league's sanctioned career record (regular season + championship bracket only; consolation games don't count) must be built from `playoff_games.csv`, not from `weekly_matchups.csv`'s playoff rows. See `scripts/build_program.py`'s `career_totals()`. |
...
## The Program's data files (2026-09-07)

Hand-built inputs for the weekly preview at `/program` (see README's
`scripts/build_program.py` section for the pipeline):

- **`divisions_2026.csv`** — the season's three divisions (Rock/Paper/Scissors) and the
  team-name → manager mapping for 2026. Hand-entered from the Yahoo settings; Yahoo's
  standings page shows division membership but the scraper has never captured it.
- **`schedule_2026.csv`** — the full 14-week schedule as entered into Yahoo, designed in
  the 2026 preseason: rivalry (all-divisional) weeks 1–3 and 12–14 mirror-reversed, with
  marquee cross-division games pinned through weeks 4–11.
- **`data/program/`** — per-issue facts JSON + writer's brief (generated by
  `build_program.py --week N`) and the hand-authored `*-copy.json` editorial files. The
  copy contract lives in the script's docstring: every quantitative claim verified against
  the facts, 25–35 word closers. Since 2026-09-15 also each week's Recap:
  `*-recap.json` (facts) and `*-recap-brief.md` from `build_recap.py --week N`, plus the
  approved `*-recap-copy.json` (a copy file with `"draft": true` never reaches the site).

## The Recap's data files (2026-09-15)

Scraped Tuesday mornings by `scripts/fetch_week_lineups.py` from the six Yahoo matchup
...
- **`results_2026.csv`** — `week, team_name, score, opponent, opponent_score, projected`,
  one row per team. The current season's results for The Program: `build_program.py`
  layers them into series and career totals (weeks before the issue only), and
  `build_recap.py` reconciles every lineup against them. `weekly_matchups.csv` stays
